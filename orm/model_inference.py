...
class StafflineSegmentationModel:
    ...
    def predict_full(
        ...
        If the longest side of *img_bgr* exceeds *max_side*, the image is
        proportionally downscaled before inference and the resulting probability
        map is bilinearly upscaled back to the original resolution.  This keeps
        the number of tiles reasonable for large scans.

        Args:
            img_bgr  : Input image (H, W, 3) uint8.
            overlap  : Tile overlap in pixels (default 64).
            max_side : Maximum side length before auto-downscaling.
                       Pass ``None`` to disable scaling (default 2048).

        Returns:
            (H, W, 3) float32 probability map at the original image resolution.
        """
        h_orig, w_orig = img_bgr.shape[:2]
        scale = 1.0
    ...
    def extract_staff_lines(
        ...
        """Extract staff-line y-coordinates from a probability map.

        The staff-line channel of *prob_map* is thresholded to produce a binary
        mask. A horizontal projection profile is then computed and peak detection
        + grouping (reusing the classical helpers) yields the final staffs.

        Args:
            prob_map     : Output of predict_full() — (H, W, 3) float32.
            conf_thresh  : Minimum staff-line probability for a pixel to be "on".
            smooth_sigma : Gaussian sigma for projection smoothing.
            peak_thresh  : Peak-detection threshold as a fraction of the max
                           projection value.

        Returns:
            List of staffs; each staff is a sorted list of 5 integer y-coords.
        """
...
class DetailedSemanticModel:
    ...
    def predict_full(
        ...
        Returns:
            (H, W, 4) float32 probability map at the original image resolution.
        """
        h_orig, w_orig = img_bgr.shape[:2]
        scale = 1.0
        if max_side is not None:
    ...
    def extract_noteheads(
        ...
        Args:
            ...
            min_area     : Minimum blob area (pixels) to be a notehead.
            max_area     : Maximum blob area (pixels).
            aspect_ratio : Acceptable (min, max) width/height ratio range.
    ...
    def extract_symbol_mask(
        ...
        """Return a binary uint8 mask (0/255) of all non-background pixels.

        Any pixel where at least one non-background channel (notehead, stem, or
        other symbol) exceeds *conf_thresh* is considered foreground.
        ...
        Returns:
            (H, W) uint8 mask where 255 = foreground symbol, 0 = background.
        """
...
def run_dual_pipeline(
    ...
    Args:
        ...
        max_side          : Maximum side length for auto-downscaling before
                            inference (default 2048). Pass None to disable.
